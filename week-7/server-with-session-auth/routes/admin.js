...
router.post("/login", async (req, res) => {
  ...
  if (passwordMatch) {
    // req.session is an object created by the express-session middleware. This object is used to store session-related data on the server.
    req.session.userId = response._id; //Create session for logged-in admin
    res.json({
      message: "Logged in successfully using session auth"
    });
  } else {
    res.status(403).json({
