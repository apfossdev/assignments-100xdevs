...
function authenticateAPIKey(req, res, next) {
    ...
    // if api key header correct then next
        const apiKey = String(req.header("100xdevs-api-key"));
        if(!apiKey) return res.status(401).json({ message: 'Invalid or missing API key' });
        if (apiKey !== VALID_API_KEY) return res.status(401).json({ message: "Invalid or missing API key" });
        next();
